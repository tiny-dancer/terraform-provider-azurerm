...
func (t HPCCacheBlobTargetResource) Exists(ctx context.Context, clients *clients.Client, state *terraform.InstanceState) (*bool, error) {
	...
	target, ok := resp.AsClfsTargetProperties()
	if !ok {
		return nil, fmt.Errorf("retrieving HPC Cache Blob Target %q (Resource Group %q) is not a Clfs Target", id.Name, id.ResourceGroup)
	}

	return utils.Bool(target.Clfs != nil), nil
}

func (HPCCacheBlobTargetResource) basic(data acceptance.TestData) string {
	template := HPCCacheBlobTargetResource{}.template(data)
	return fmt.Sprintf(`
%s
...
func (HPCCacheBlobTargetResource) namespace(data acceptance.TestData) string {
	template := HPCCacheBlobTargetResource{}.template(data)
	return fmt.Sprintf(`
%s
...
func (HPCCacheBlobTargetResource) requiresImport(data acceptance.TestData) string {
	template := HPCCacheBlobTargetResource{}.basic(data)
	return fmt.Sprintf(`
%s
