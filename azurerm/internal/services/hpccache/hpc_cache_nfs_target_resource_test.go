...
func (HPCCacheNFSTargetResource) Exists(ctx context.Context, clients *clients.Client, state *terraform.InstanceState) (*bool, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("retrieving HPC Cache Blob Target %q (Resource Group %q) %v", id.Name, id.ResourceGroup, err)
	}

	target, ok := resp.AsNfs3TargetProperties()
	if !ok {
		return nil, fmt.Errorf("retrieving HPC Cache Blob Target %q (Resource Group %q) is not a Clfs Target", id.Name, id.ResourceGroup)
	}

	return utils.Bool(target.Nfs3 != nil), nil
}

func (HPCCacheNFSTargetResource) basic(data acceptance.TestData) string {
	template := HPCCacheNFSTargetResource{}.template(data)
	return fmt.Sprintf(`
%s
...
func (HPCCacheNFSTargetResource) usageModel(data acceptance.TestData) string {
	template := HPCCacheNFSTargetResource{}.template(data)
	return fmt.Sprintf(`
%s
...
func (HPCCacheNFSTargetResource) namespaceJunction(data acceptance.TestData) string {
	template := HPCCacheNFSTargetResource{}.template(data)
	return fmt.Sprintf(`
%s
...
func (HPCCacheNFSTargetResource) requiresImport(data acceptance.TestData) string {
	template := HPCCacheNFSTargetResource{}.basic(data)
	return fmt.Sprintf(`
%s
...
func (r HPCCacheNFSTargetResource) template(data acceptance.TestData) string {
	return fmt.Sprintf(`
%s
